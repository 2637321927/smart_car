...
void PID_control_test(float error)
{
    ...
    ls_atim_pwm pwm2(ATIM_PWM1_PIN82, 100, 0);//初始化PWM，初始占空比为0
     ...
     int pwm1_duty1 = (500 + diffrential < 1000) ? (500 + diffrential) : 5000;//从500开始，差分输入增加pwm1的占空比,并且做了限幅（不得大于5000）
      int pwm2_duty1 = (500 - diffrential > 0) ? (500 - diffrential) : 0;//从500开始，差分输入减少pwm2的占空比，并且做了限幅（不得小于0）
