 ...
 * @参数说明：mux         :  复用 GPIO 引脚
 * @函数返回：无
 * @调用方法：AtimPwm MyPwm(81, 1, LS_ATIM_INVERSED, 2000000 - 1, 150000);
 ...
 * @参数说明：cycle  :  周期值
 * @函数返回：无
 * @调用方法：MyPwm.SetPeriod(3200000);
 * @备注说明：假设要设置 50Hz 周期，那该参数应填：
 *           160000000 / 50 = 3200000
 QQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQ*/
void AtimPwm::SetPeriod(uint32_t cycle)
{
    this->Disable();
    this->Period = cycle;
    LS_writel(this->ATIM_ARR, this->Period);
    this->Enable();
}
...
void AtimPwm::SetDutyCycle(uint32_t duty_cycle)
{
    ...
    LS_writel(this->ATIM_CCRx, this->Duty_cycle);
