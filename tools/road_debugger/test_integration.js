...
async function main() {
  ...
  try {
    ...
    assert(appResponse.text.includes("key: 'dbReturnBias', label: '回赛道预偏角', min: 0, max: 91, step: 1, unit: 'deg', defaultValue: 24"));
    ...
    assert(appResponse.text.includes("key: 'dbTurnRps', label: '转出阶段前进基准速度', min: 0, max: 40, step: 1, unit: 'RPS', defaultValue: 10"));
    ...
    assert(appResponse.text.includes("key: 'dbBrakePwm', label: '主动制动反向PWM', min: 0, max: 7000, step: 50, defaultValue: 7000, hardMax: true"));
    assert(appResponse.text.includes("key: 'dbBrakeTimeout', label: '制动超时', min: 1, max: 2000, step: 10, unit: 'ms', defaultValue: 301"));
