...
static struct termios old_tio;
 ...
 volatile  int mid;
 const uint8_t VOFA_FRAME_TAIL[4] = {0x00, 0x00, 0x80, 0x7f};
...
cv::Mat M = (cv::Mat_<float>(3, 3) <<
    -1.084902477207633, -3.23791898753001, 365.2607587309688,
    0.2052690839030848, -5.416435650983655, 351.2464438241162,
    0.0008692395138381539, -0.01996591663549253, 1);

cv::Mat M_Reverse = (cv::Mat_<float>(3, 3) <<
    -0.8708532890511503, 2.211793947691175, -458.7962254960458,
    -0.05457321064380306, 0.7649668124006941, -248.7584201730731,
    -0.0003326240846461663, 0.01335068490950946, -3.567886071551437);

// Auxiliary calibration values
// ground_width_m = 0.66
#define M2PIX 208.7878787878788 // 米转像素
...
float thres = 110;            // 固定二值化阈值（判断黑线/背景）
float block_size = 5;         // 自适应阈值的窗口大小
float clip_value = 2;         // 自适应阈值减去的偏移量
float track_min_y = 100;   // 越小 → 巡得越远
float track_max_y = 232;  // 越大 → 巡到最底部
float begin_x = 16;           // 巡线起始点 水平偏移
float begin_y = 230;          // 巡线起始点 垂直位置（靠近车底）
...
//set_terminal_nonblock();
    ...
    udp_receive.udp_client_init("192.168.43.9", 8888);
...
// std::lock_guard<std::mutex> lock(g_mutex);
 cv::Mat frame = cam.get_gray_frame();
        if (frame.empty()) {
...
snprintf(encoder_str, sizeof(encoder_str),
       ...
       pwm1_duty_rps, pwm2_duty_rps, encoder_1, encoder_2, latest_error
,Road_Wide[25]);
    /*
        float data[5] = {
        (float) pwm1_duty_rps,  // 通道0
        (float)pwm2_duty_rps,  // 通道1
        encoder_1,            // 通道2
        encoder_2,            // 通道3
        (float)mid,      // 通道4
    };
